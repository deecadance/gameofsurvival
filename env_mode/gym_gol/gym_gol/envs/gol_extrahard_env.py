...
class GolExtraHardEnv(gym.Env):
  ...
  def __init__(self,epochs=200,world_size=50,group_size=4,see_size=20,filling=0.50,fixed_start=True,seed=42):
    ...
    self.seed = seed                ## Random seed (start from pre-defined config?)
    ...
    self.next_world = self.worlf    ## World copy
    ...
    self.observation_space = \
      spaces.Box(shape=(self.see_size, self.see_size))  
    ...
    def _get_reward(self):
      if not np.any(self.next_world[self.group_index_1:self.group_index_2,self.group_index_1:self.group_index_2]):
        got_reward = -self.group_size*self.group_size*10
        self.done = 1
      else:
        got_reward = np.sum(self.next_world[self.group_index_1:self.group_index_2,self.group_index_1:self.group_index_2])
        self.done = 0
    return got_reward

  def _get_state(self):
    return self.next_world[self.sight_index_1:self.sight_index_2,self.sight_index_1:self.sight_index_2]
  ...
  def _init_grid(self):
    return np.random.choice([0,1], self.size*self.size, 
          p=[1-self.filling, self.filling]).reshape(self.size,self.size)
  ...
  def _set_to_grid(population, world_size):
    new_grid = np.zeros((world_size, world_size))
